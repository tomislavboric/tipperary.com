...
if ( !class_exists('FG_Drupal_to_WordPress_Modules_Check', false) ) {
	...
	class FG_Drupal_to_WordPress_Modules_Check {
		...
		private function check_drupal8_entity_reference_fields() {
			if ( $this->plugin->table_exists('config') ) {
				$prefix = $this->plugin->plugin_options['prefix'];
				$sql = "
					SELECT c.name, c.data
					FROM ${prefix}config c
					WHERE c.name LIKE 'field.field.node.%.field_%'
				";
		...
		/**
		 * Check if the database contains some files started by a given protocol (s3, soundcloud, youtube)
		 * 
		 * @since 1.39.0
		 * 
		 * @param string $protocol Drupal protocol (s3, soundcloud, youtube)
